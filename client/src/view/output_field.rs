...
impl OutputField {
    pub fn new(rect: Rectangle, text: String, align: Align) -> OutputField {
        OutputField {
            id: ID_FEEDER.next(),
            rect,
            children: Vec::new(),
    ...
    pub fn event(mut self, event: Option<Event>) -> OutputField {
        self.event = event;
        self
    }

    pub fn hold_event(mut self, event: Option<Event>) -> OutputField {
    ...
    fn handle_event(&mut self, evt: &Event, _hub: &Hub, bus: &mut Bus, rq: &mut RenderQueue, _context: &mut Context) -> bool {
        match *evt {
            ...
            Event::Gesture(GestureEvent::Tap(center)) if self.rect.includes(center) => {
                ...
                true 
                //handled bc did something, but sent another event to be handled...?
                //always handled true as long as the tap point is inside OutputField rect...
                //if event field is not empty send the event field back to parent, which is gui struct. but...
                //event field is only created during new instance. you can turn it into a button?
            },
