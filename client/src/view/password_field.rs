...
impl Password_Field {
    pub fn new(rect: Rectangle, view_id: ViewId) -> Password_Field {
        Password_Field {
            id: ID_FEEDER.next(),
            rect,
            children: Vec::new(),
    ...
    pub fn border(mut self, border: bool) -> Password_Field {
        self.border = border;
        self
    }

    pub fn placeholder(mut self, placeholder: &str) -> Password_Field {
        self.placeholder = placeholder.to_string();
        self
    }

    pub fn text(mut self, text: &str, context: &mut Context) -> Password_Field {
...
impl View for Password_Field {
    fn handle_event(&mut self, evt: &Event, hub: &Hub, bus: &mut Bus, rq: &mut RenderQueue, context: &mut Context) -> bool {
        match *evt {
            ...
            // Event::Gesture(GestureEvent::Tap(center)) if !self.rect.includes(center) && self.focused => { //make text submit no matter where you tap
            //     bus.push_back(Event::Submit(self.view_id, self.text.clone()));
            //     // hub.send(Event::Submit(self.view_id, self.text.clone()));
    ...
    fn render(&self, fb: &mut dyn Framebuffer, _rect: Rectangle, fonts: &mut Fonts) {
        ...
        let (mut plan, foreground) = if self.text.is_empty() {
            ...
            let hidden: String =  "*".repeat(self.text.chars().count());
            (font.plan(&hidden, None, Some(&["-liga".to_string()])),
            TEXT_NORMAL[1])
        };
